function [ z ] = dtree_classifier_expdata(term_weighting_scheme,feature_size,feature_selection_method,dataset_type,tokenization_type,usestopwordremoval,uselowercase,usestemming)
...
lentrain=length(train_matrix(:,1));
actual_labels_for_train=repmat(cellstr(''), 1,lentrain);
for k=1:lentrain
    actual_labels_for_train(1,k)=cellstr(num2str(topic_matrix(k,1)));
end

decision_tree=classregtree(train_matrix,actual_labels_for_train);
predicted = eval(decision_tree,test_matrix);
lentest=length(test_matrix(:,1));
predicted_labels=zeros(1,lentest);

for k=1:lentest
    for l=1:length(dataset_class_idx)
        if (strcmp(predicted(k,1),num2str(l))==1)
            predicted_labels(1,k)=l;
        end
    end
end
...
if dataset_type==DATASET_REUTERS
    resfoldername='experiments_with_reuters_21578';
    finalresults=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\reutersexperiments\dtree\',finalresults);
    finalmatrix=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\reutersexperiments\dtree\',finalmatrix);
    filename=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\reutersexperiments\dtree\',filename);
elseif dataset_type==DATASET_MILLIYET
    resfoldername='experiments_with_milliyet_collection';
...
elseif dataset_type==DATASET_SPAM
    resfoldername='experiments_with_spam';
    finalresults=strcat('spamexperiments\dtree\',finalresults);
    finalmatrix=strcat('spamexperiments\dtree\',finalmatrix);
    filename=strcat('spamexperiments\dtree\',filename);         
elseif dataset_type==DATASET_NEWS2C
    resfoldername='experiments_with_news2c';
...
result_fpath=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\',resfoldername,'\dtreeres.txt');
