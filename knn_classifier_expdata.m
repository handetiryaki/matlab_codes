function [ z ] = knn_classifier_expdata(term_weighting_scheme,feature_size,feature_selection_method,dataset_type,tokenization_type,usestopwordremoval,uselowercase,usestemming)
...
% [~,predicted_labels] = testk(dataset_train,K,dataset_test);
% 
% predicted_labels=predicted_labels';

K = 1;
Mdl = fitcknn(train_matrix,topic_matrix,'NumNeighbors',K,...
    'NSMethod','exhaustive','Distance','cosine');
predicted_labels = predict(Mdl,test_matrix);
predicted_labels = predicted_labels';
...
if dataset_type==DATASET_REUTERS
    resfoldername='experiments_with_reuters_21578';
    finalresults=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\reutersexperiments\knn\',finalresults);
    finalmatrix=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\reutersexperiments\knn\',finalmatrix);
    filename=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\reutersexperiments\knn\',filename);
elseif dataset_type==DATASET_MILLIYET
    resfoldername='experiments_with_milliyet_collection';
...
elseif dataset_type==DATASET_SPAM
    resfoldername='experiments_with_spam';
    finalresults=strcat('spamexperiments\knn\',finalresults);
    finalmatrix=strcat('spamexperiments\knn\',finalmatrix);
    filename=strcat('spamexperiments\knn\',filename);         
elseif dataset_type==DATASET_NEWS2C
    resfoldername='experiments_with_news2c';
...
result_fpath=strcat('C:\googledriveakuysal\academic\kod\term_weighting_experiments\',resfoldername,'\knnres.txt');


fresid = fopen(result_fpath, 'a');
fprintf(fresid,'%d %d %d %d %d %d %d %d %d %d %6.3f %6.3f %d\n',term_weighting_scheme,feature_selection_method,length(term_feature),tokenization_type, usestopwordremoval,uselowercase,usestemming,feature_size,num_true_classifications,num_false_classifications,micro_F1,macro_F1,K);
fclose(fresid);
